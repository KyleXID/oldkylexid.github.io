---
layout: post
title:  "Python lambda, dic sort, git rebase 정리"
date: 2019-05-22
categories: TIL
---
## Python 람다(lambda)
...
## 딕셔너리 정렬
----------
딕셔너리 정렬 방법에는 과거에 다루었던 operater의 itemgetter를 사용하는 방법 외에 다른 방법들이 존재한다.
...
### 2.items() 정렬
딕셔너리에 .items() 메서드를 사용해주면 `key : value`의 형태를 `(key, value)`의 형태로 바꿔서 list를 만들어 준다.  
...
위는 딕셔너리에 접근하였고, 아래는 튜플에 접근하여 사용하기때문에, 해당 value값이 위치한 곳에 접근하기 위함임을 알아두어야 한다.
